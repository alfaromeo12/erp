<%@ Page Language="VB" MasterPageFile="~/MXPageNoBusy.master" AutoEventWireup="false" CodeFile="AccDocDeliver.aspx.vb" Inherits="Txn_AccReceivedDeliver" title="Collateral Received by Account" EnableEventValidation="FALSE"  MaintainScrollPositionOnPostback="true"%>
            ...
            <tr>
                ...
                <td>
                    <asp:DropDownList ID="ddlAreaSearch" runat="server">
                        ...
                        ServicePath="~\Param\WebService.asmx" TargetControlID="ddlAreaSearch" BehaviorID="BehavArea">
                ...
                <td>
                    ...
                    </asp:DropDownList><ajaxToolkit:CascadingDropDown ID="ajaxCDDBranch" runat="server" Category="BRANCH"
                        LoadingText="[Loading Branch...]" PromptText="[DISPLAY ALL]" ServiceMethod="GetDropDownBranchByArea"
                        ServicePath="~\Param\WebService.asmx" TargetControlID="ddlBranchSearch" ParentControlID="ddlAreaSearch" BehaviorID="BehavBranch">
                    </ajaxToolkit:CascadingDropDown>
                </td>
            </tr>
            <tr>
        ...
        <asp:UpdatePanel ID="UPGrid" runat="server">
            <ContentTemplate>
                <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                    AutoGenerateColumns="False" Caption="Document Account To Deliver" 
                ...
                <asp:SqlDataSource ID="sdsGrid" runat="server" 
                    ...
                    SelectCommand="SELECT TOP (500) '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCH, CUSTOMER.CUSTID, ACCOUNT.ACCID, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, '(' + CAST(ACCOUNT.ACCSTATUS AS varchar(20)) + ') ' + dbo.f_getParamGlobalName('ACCSTATUS ', ACCOUNT.ACCSTATUS) AS ACCSTATUSNAME, '(' + CAST(COLLATERAL.BPKBSTATUS AS VARCHAR(20)) + ') ' + dbo.f_getParamGlobalName('BPKBSTATUS', COLLATERAL.BPKBSTATUS) AS BPKBSTATUSNAME, CAST(CASE WHEN BPKBSTATUS IN (2) THEN CASE WHEN ACCSTATUS NOT IN (0 , 9) THEN 1 ELSE 0 END ELSE 0 END AS BIT) AS RECEIVECHECK, COLLATERAL.LOCID, COLLATERAL.OBJID, COLLATERAL.BPKBNO, COLLATERAL.BPKBDATE, COLLATERAL.BPKBNAME, ACCOUNT.APPID, ACCOUNT.ACCOUNTNAME FROM ACCOUNT WITH (NOLOCK) INNER JOIN CUSTOMER WITH (NOLOCK) ON ACCOUNT.LOCIDCUST = CUSTOMER.LOCIDCUST AND ACCOUNT.CUSTID = CUSTOMER.CUSTID INNER JOIN BRANCH WITH (NOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID INNER JOIN COLLATERAL WITH (NOLOCK) ON ACCOUNT.LOCID = COLLATERAL.LOCID AND ACCOUNT.ACCID = COLLATERAL.ACCID WHERE (ACCOUNT.ACCSTATUS &lt;&gt; 0) AND (ACCOUNT.RELEASESTS = 0)">
                </asp:SqlDataSource>
            </ContentTemplate>
    </asp:UpdatePanel>
        <asp:Panel ID="pnlInit" runat="server" 
        GroupingText="Deliver Collateral Document by Checklist" Width="100%" 
            ...
            <table cellpadding="0" cellspacing="0" width="100%">
                ...
                <tr>
                    <td class="FooterStyle">
                        Deliver&nbsp;Date</td>
                    <td class="FooterStyle">
                        <asp:TextBox ID="txtDeliverDate" runat="server" CssClass="required" 
                            ValidationGroup="ValSave" Width="72px"></asp:TextBox>
                        <asp:Image ID="imgDeliver" runat="server" ImageUrl="~/images/grid.gif" 
                            Style="cursor: pointer" ToolTip="Calendar" />
                        <asp:RequiredFieldValidator ID="reqDeliverDate" runat="server" 
                            ControlToValidate="txtDeliverDate" Display="None" 
                            ErrorMessage="DELIVER DATE must be entry...!" SetFocusOnError="True" 
                        ...
                        <ajaxToolkit:ValidatorCalloutExtender ID="ajaxVCEPReceiveDate" runat="server" 
                            HighlightCssClass="ErrHighLight" TargetControlID="reqDeliverDate">
                        </ajaxToolkit:ValidatorCalloutExtender>
                        <ajaxToolkit:CalendarExtender ID="ajaxCEReceive" runat="server" Animated="False" 
                            Format="dd/MM/yyyy" PopupButtonID="imgDeliver" 
                            TargetControlID="txtDeliverDate" PopupPosition="TopRight">
                        </ajaxToolkit:CalendarExtender>
                        <ajaxToolkit:MaskedEditExtender ID="ajaxMEEReceiveDate" runat="server" 
                            Mask="99/99/9999" MaskType="Date" TargetControlID="txtDeliverDate">
                        </ajaxToolkit:MaskedEditExtender>
                    </td>
                </tr>
                <tr>
                    <td class="HeaderStyle" colspan="2">
                        <asp:Button ID="btnSaveCheck" runat="server" 
                            ...
                            Text="Deliver By Check List" ValidationGroup="ValSave" Width="168px" />
                    </td>
                </tr>
            </table>
        </asp:Panel>
