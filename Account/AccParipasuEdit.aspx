<%@ Page Language="VB" MasterPageFile="~/MXPageNoBusy.master" AutoEventWireup="false" CodeFile="AccParipasuEdit.aspx.vb" Inherits="Txn_ParipasuEdit" title="Account Paripasu Edit" EnableEventValidation="FALSE" MaintainScrollPositionOnPostback="true"%>
    ...
    <asp:MultiView ID="MVMain" runat="server">
        <asp:View ID="vSearch" runat="server">
            ...
            <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                AutoGenerateColumns="False" Caption="Account List" 
                DataKeyNames="LOCID,ACCID" 
            ...
            <asp:SqlDataSource ID="sdsGrid" runat="server" 
                ...
                SelectCommand="SELECT TOP (500) ACCOUNT.ACCOUNTNAME, CUSTOMER.CUSTID, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCHNAME, ACCOUNT.APPID, ACCOUNT.LOCID, ACCOUNT.ACCID, ACCOUNT.OSPRINCIPALREAL, ACCOUNT.INSTALLMENT FROM ACCOUNT WITH (NOLOCK) INNER JOIN CUSTOMER WITH (NOLOCK) ON ACCOUNT.LOCIDCUST = CUSTOMER.LOCIDCUST AND ACCOUNT.CUSTID = CUSTOMER.CUSTID INNER JOIN BRANCH WITH (NOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID WHERE (ACCOUNT.ACCSTATUS = 1)">
            </asp:SqlDataSource>
        ...
        <asp:View ID="vEdit" runat="server">
            ...
            <asp:Panel ID="pnlCollateral" runat="server" Width="100%">
                ...
                <input
						id="Button16" noenter onclick="Paripasu();" style="background-position: left;
						background-image: url('../images/search.gif'); width: 116px; cursor: pointer; background-repeat: no-repeat;
						 text-align: right" type="button" value="New Paripasu" />
                <asp:GridView ID="gvParipasu" runat="server" AutoGenerateColumns="False" 
                    Caption="ACCOUNT PARIPASU (TO)" DataKeyNames="LOCIDX,ACCIDX,PARIPASUID" 
...
function Paripasu() {
    ...
    var WinRef = window.open("../FrontEnd/AppParipasu.aspx?Action=Find&LOCID=" + LOCID + "&ACCID=" + ACCID, "ParipasuFind", "height=500,width=650,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable")
    WinRef.focus()
    return (false);
}

function SetParipasu(LOCID, ACCID) {
    SetValueTxtByClientID("<%=hidLOCIDPARIPASU.ClientID%>", LOCID);
    SetValueTxtByClientID("<%=hidACCIDPARIPASU.ClientID%>", ACCID);
    document.forms[0].action = "AccParipasuEdit.aspx?ACTION=PARIPASU";
