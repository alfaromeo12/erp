...
Partial Class Param_CollateralActivity
    ...
    Private Sub SetValidation()
        ...
        oTextBox = CType(fvData.FindControl("txtCollateralCode"), TextBox)
        If oTextBox IsNot Nothing Then
            SetObjectValidation(oTextBox, False, "uppercase nospace")
            oTextBox.Focus()
        End If
        oTextBox = CType(fvData.FindControl("txtCollateralName"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "uppercase")

        oTextBox = CType(fvData.FindControl("txtCollateralFee"), TextBox)
    ...
    Protected Sub sdsForm_Inserted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsForm.Inserted
        If e.Command.Parameters("@retval").Value = -1 Then
            ...
            lblError.Text = "Collateral Code : " & e.Command.Parameters("@COLLACTCODE").Value & " already exists..!"
        Else
            gvData.SelectedIndex = -1
            lblError.Visible = False
            fvData.Caption = ""
