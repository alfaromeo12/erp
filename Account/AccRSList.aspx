<%@ Page Language="VB" MasterPageFile="~/MXPageNoBusy.master" AutoEventWireup="false" CodeFile="AccRSList.aspx.vb" Inherits="Txn_AccRS" title="Account Reschedule List" EnableEventValidation="FALSE"  MaintainScrollPositionOnPostback="true"%>
        ...
        WatermarkText="%%">
            ...
            <tr>
                ...
                <td>
                    Installment Type</td>
                <td>
                    <asp:DropDownList ID="ddlInstallmentType" runat="server" 
                        DataSourceID="sdsInstallmentType" DataTextField="NAME" DataValueField="CODE">
                    </asp:DropDownList>
                    <asp:SqlDataSource ID="sdsInstallmentType" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:connection %>" 
                        
                        SelectCommand="SELECT NULL AS CODE, '[DISPLAY ALL]' AS NAME UNION ALL SELECT CODE, CODE + ' - ' + NAME AS NAME FROM PARAMGLOBAL WITH (NOLOCK) WHERE (TYPE = 'INSTALLMENTTYPE') AND (CODE &lt;&gt; '9')">
                    </asp:SqlDataSource>
                </td>
            </tr>
        </table>
        ...
        <asp:UpdatePanel ID="UPGrid" runat="server">
            <ContentTemplate>
                <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                    AutoGenerateColumns="False" Caption="Account List for Reschedule" 
                ...
                <asp:SqlDataSource ID="sdsGrid" runat="server" 
                    ConnectionString="<%$ ConnectionStrings:connection %>" 
                    SelectCommand="SELECT TOP (500) ACCOUNT.ACCOUNTNAME, CUSTOMER.CUSTID, ACCOUNT.ACCID, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, ACCOUNT.EFFECTIVERATE, ACCOUNT.INSTALLMENTREAL, ACCOUNT.PRINCIPALTOTAL, ACCOUNT.TENOR, '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCHNAME, ACCOUNT.LOCID, ACCOUNT.APPID FROM ACCOUNT WITH (NOLOCK) INNER JOIN CUSTOMER WITH (NOLOCK) ON ACCOUNT.LOCIDCUST = CUSTOMER.LOCIDCUST AND ACCOUNT.CUSTID = CUSTOMER.CUSTID INNER JOIN BRANCH WITH (nOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID WHERE (ACCOUNT.ACCSTATUS = 1) AND (NOT EXISTS (SELECT 'X' AS Expr1 FROM RESCHEDULE WITH (NOLOCK) WHERE (ACCOUNT.LOCID = LOCID) AND (ACCOUNT.ACCID = ACCID) AND (RSSTATUS = 0))) AND (ACCOUNT.INSTALLMENTTYPE &lt;&gt; 9)">
                </asp:SqlDataSource>
            </ContentTemplate>
    </asp:UpdatePanel>
...
function EditApp(LOCID, ACCID) {
    ...
    window.location = "AccRSEntry.aspx?LOCID=" + LOCID + "&ACCID=" + ACCID + "&RSType=" + RSType;
}
