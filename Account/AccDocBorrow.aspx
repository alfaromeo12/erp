<%@ Page Language="VB" MasterPageFile="~/MXPageNoBusy.master" AutoEventWireup="false" CodeFile="AccDocBorrow.aspx.vb" Inherits="Txn_AccBorrow" title="Collateral Document Borrow Process" EnableEventValidation="FALSE"  MaintainScrollPositionOnPostback="true"%>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphMain" Runat="Server">
        ...
        <asp:UpdatePanel ID="UPGrid" runat="server">
            <ContentTemplate>
                <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                    AutoGenerateColumns="False" Caption="Document Account  to Borrow" DataKeyNames="LOCID,OBJID" 
                    DataSourceID="sdsGrid" RowHeaderColumn="ACCID" Width="140%">
                    <Columns>
                        <asp:TemplateField>
                            <ItemTemplate>
                ...
                <asp:SqlDataSource ID="sdsGrid" runat="server" 
                    ...
                    SelectCommand="SELECT TOP (500) '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCH, CUSTOMER.CUSTID, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, '(' + CAST(ACCOUNT.ACCSTATUS AS varchar(20)) + ') ' + dbo.f_getParamGlobalName('ACCSTATUS ', ACCOUNT.ACCSTATUS) AS ACCSTATUSNAME, '(' + CAST(COLLATERAL.BPKBSTATUS AS VARCHAR(20)) + ') ' + dbo.f_getParamGlobalName('BPKBSTATUS', COLLATERAL.BPKBSTATUS) AS BPKBSTATUSNAME, CAST(CASE WHEN BPKBSTATUS = 2 THEN CASE WHEN ACCSTATUS NOT IN (0 , 9) THEN 1 ELSE 0 END ELSE 0 END AS BIT) AS RECEIVECHECK, COLLATERAL.LOCID, COLLATERAL.OBJID, COLLATERAL.BPKBNO, COLLATERAL.BPKBDATE, COLLATERAL.BPKBNAME, ACCOUNT.ACCID, ACCOUNT.APPID, ACCOUNT.ACCOUNTNAME FROM ACCOUNT WITH (NOLOCK) INNER JOIN CUSTOMER WITH (NOLOCK) ON ACCOUNT.LOCIDCUST = CUSTOMER.LOCIDCUST AND ACCOUNT.CUSTID = CUSTOMER.CUSTID INNER JOIN BRANCH WITH (NOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID INNER JOIN COLLATERAL WITH (NOLOCK) ON ACCOUNT.LOCID = COLLATERAL.LOCID AND ACCOUNT.ACCID = COLLATERAL.ACCID WHERE (ACCOUNT.ACCSTATUS &lt;&gt; 0)">
                </asp:SqlDataSource>
            </ContentTemplate>
    </asp:UpdatePanel>
    <br />
    ...
    <asp:Panel ID="pnlBorrow" runat="server" Width="100%" CssClass="FooterStyle">
        <table cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td class="RowStyle">
                    Borrow By JF (Bank)</td>
                <td class="RowStyle">
                    <asp:RadioButton ID="rbYes" runat="server" GroupName="BorrowJF" Text="Yes" />
                    <asp:RadioButton ID="rbNo" runat="server" GroupName="BorrowJF" Text="No" />
                </td>
            </tr>
            <tr>
                <td class="AlternatingRowStyle">
                    Borrow Purpose</td>
                <td class="AlternatingRowStyle">
                    <asp:DropDownList ID="ddlBorrowPurpose" runat="server" CssClass="required" DataSourceID="sdsBorrowPurpose"
                        DataTextField="BRPNAME" DataValueField="BRPID">
                    </asp:DropDownList><asp:RequiredFieldValidator ID="reqBRP" runat="server" ControlToValidate="ddlBorrowPurpose"
                        Display="None" ErrorMessage="BORROW PURPOSE must be entry...!" SetFocusOnError="True"
                        ValidationGroup="ValSave"></asp:RequiredFieldValidator><ajaxToolkit:ValidatorCalloutExtender
                                    ID="ajaxVCEBRP" runat="server" HighlightCssClass="ErrHighLight" TargetControlID="reqBRP">
                        </ajaxToolkit:ValidatorCalloutExtender>
                    <asp:SqlDataSource ID="sdsBorrowPurpose" runat="server" ConnectionString="<%$ ConnectionStrings:connection %>"
                        SelectCommand="SELECT NULL AS BRPID, NULL AS BRPCODE, '[BORROW PURPOSE]' AS BRPNAME UNION ALL SELECT BRPID, BRPCODE, '(' + BRPCODE + ') ' + BRPNAME AS BRPNAME FROM BORROWPURPOSE WHERE (DeleteSts = 0)&#13;&#10;ORDER BY BRPCODE">
                    </asp:SqlDataSource>
                </td>
            </tr>
            <tr>
                <td class="RowStyle">
                    Borrower</td>
                <td class="RowStyle">
                    <asp:TextBox ID="txtBorrower" runat="server" CssClass="required" MaxLength="50" Width="216px"></asp:TextBox><asp:RequiredFieldValidator
                        ID="reqBorrower" runat="server" ControlToValidate="txtBorrower" Display="None"
                        ErrorMessage="BORROWER must be entry...!" SetFocusOnError="True" ValidationGroup="ValSave"></asp:RequiredFieldValidator><ajaxToolkit:ValidatorCalloutExtender
                                    ID="ajaxVCEBorrower" runat="server" HighlightCssClass="ErrHighLight" TargetControlID="reqBorrower">
                        </ajaxToolkit:ValidatorCalloutExtender>
                </td>
            </tr>
            <tr>
