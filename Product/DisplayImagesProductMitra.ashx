...
Public Class DisplayImagesVB : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        Dim qtnId As Int32
        If Not context.Request.QueryString("PRODUCTMITRAID") Is Nothing Then
            qtnId = Convert.ToInt32(context.Request.QueryString("PRODUCTMITRAID"))
        Else
    ...
    Public Function ShowEmpImage(ByVal qtnId As Integer) As Stream
        ...
        Dim sql As String = "SELECT IMAGEPRODUCT FROM PRODUCTMITRA WHERE PRODUCTMITRAID = @PRODUCTMITRAID"
        Dim cmd As SqlCommand = New SqlCommand(sql, connection)
        cmd.CommandType = CommandType.Text
        cmd.Parameters.AddWithValue("@PRODUCTMITRAID", qtnId)
        connection.Open()
