...
Partial Class Param_InsCompanyApp
    ...
    Private Sub SetValidation()
        ...
        oTextBox = CType(fvData.FindControl("txtInsuranceGL"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "int", "[0-9]")

        oTextBox = CType(fvData.FindControl("txtInsuranceDati"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "int", "[0-9]")
    ...
    Protected Sub fvData_ItemCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.FormViewCommandEventArgs) Handles fvData.ItemCommand
        ...
        ElseIf e.CommandName = "Approve" Or e.CommandName = "Reject" Then
            ...
            Try
                ...
                intRetVal = objDB.ExecSPReturnInteger("INSCOMPANYAPPROVAL", _
                    objDB.MP("@INSCOMPANYID", Data.SqlDbType.Int, fvData.DataKey.Item(0)), _
            ...
            Catch ex As Exception
                lblError.Visible = True
                lblError.Text = "Insurance Reject/Approval Error: " & ex.Message & "<br>PLEASE CHECK DETAIL ERROR MESSAGE...!"
            Finally
                objDB.Close()
                objDB = Nothing
            End Try
