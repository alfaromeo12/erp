...
Partial Class Txn_ReceivedAcc
        ...
        Try
            If Not IsPostBack() Then
                oSDR = objDB.CreateSDRFromSQLSelect("SELECT APPVALUE, " & _
                  "CONVERT(VARCHAR(20), DateAdd(day, 3, dbo.f_getAppDate()), 103) RETURNDATE, dbo.f_getAppSetting('PAYHANDLEBRANCEONLY')PAYHANDLEBRANCEONLY, CAST(dbo.f_IsUserBranch(" & txtUID.Value & ") as VARCHAR(10)) ISUSERBRANCH  FROM APPSETTING WITH(NOLOCK) WHERE APPCODE='APPDATE'")
                If oSDR.Read() Then
                    txtAppDate.Value = oSDR("APPVALUE").ToString()
                    hidIsUserBranch.Value = oSDR("ISUSERBRANCH").ToString()
                    'txtPAYHANDLEBRANCEONLY.Value = oSDR("PAYHANDLEBRANCEONLY").ToString()
                    txtReceiveDate.Text = oSDR("RETURNDATE").ToString()
                End If
                oSDR.Close()
                oSDR = Nothing
                hidBranch.Value = Request.QueryString("BRANCH")
                ddlAccStatus.SelectedValue = "1"
                ddlCollStatus.SelectedValue = "1"
                ...
                'If hidIsUserBranch.Value = "1" And hidBranch.Value = "1" Then 'user cabang
                '    If txtPAYHANDLEBRANCEONLY.Value = "1" Then
    ...
    Private Sub DataSearch()
        If hidIsUserBranch.Value = "1" And hidBranch.Value = "1" Then 'jika user cabang, filter berdasarkan cabang yang berhak saja
            'If txtPAYHANDLEBRANCEONLY.Value = "1" Then
    ...
    Protected Sub btnSaveCheck_Click(sender As Object, e As System.EventArgs) Handles btnSaveCheck.Click
                ...
                If oCheckBox IsNot Nothing Then
                    If oCheckBox.Checked Then
                        ...
                        objDB.ExecSP("COLLATERALReceive", _
                            ...
                            objDB.MP("@RECEIVEDATE", Data.SqlDbType.DateTime, ConvertDate(txtReceiveDate.Text)),
                            objDB.MP("@DOCSHELFID", Data.SqlDbType.BigInt, ddlDocShelf.SelectedValue),
        ...
        Catch ex As Exception
            lblMessage.Visible = True
            lblMessage.Text = "ERROR SAVE COLLATERAL RECEVED BY ACCOUNT..!<BR>PLEASE CHECK DETAIL ERROR MESSAGE:<BR>" & ex.Message
            lblMessage.Focus()
        Finally
            objDB.Close()
            objDB = Nothing
