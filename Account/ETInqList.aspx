<%@ Page Language="VB" MasterPageFile="~/MXPageNoBusy.master" AutoEventWireup="false" CodeFile="ETInqList.aspx.vb" Inherits="ET_Inq" title="ET Inquiry List" EnableEventValidation="FALSE"  MaintainScrollPositionOnPostback="true"%>
        ...
        WatermarkText="%%">
            ...
            <tr>
                ...
                <td>
                    ET Calc Type</td>
                <td>
                    <asp:DropDownList ID="ddlETCalTypeSearch" runat="server" 
                        DataSourceID="sdETCalcType" DataTextField="NAME" DataValueField="CODE">
                    </asp:DropDownList>
                    <asp:SqlDataSource ID="sdETCalcType" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:connection %>" 
                        SelectCommand="SELECT NULL AS CODE, '[DISPLAY ALL]' AS NAME UNION ALL SELECT CODE, CODE + ' - ' + NAME AS NAME FROM PARAMGLOBAL WITH (NOLOCK) WHERE (TYPE = 'ETCALCTYPE')">
                    </asp:SqlDataSource>
                </td>
            </tr>
            <tr>
            ...
            <ContentTemplate>
                <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                    ...
                    <Columns>
                        <asp:TemplateField>
                            <ItemTemplate>
                        <img src="../images/edit.gif" style="cursor: pointer" onclick="ShowET('<%#eval("LOCID")%>', '<%#eval("ACCID")%>', '<%#eval("ETID")%>')" alt="Edit : <%#eval("LOCID")%> - <%#eval("ACCID")%>" />
                            </ItemTemplate>
                        ...
                        <asp:BoundField DataField="ETDATE" DataFormatString="{0:dd/MM/yyyy}" 
                            HeaderText="ET&lt;br&gt;Date" HtmlEncode="False" SortExpression="ETDATE" />
                        <asp:BoundField DataField="ETTXNDATE" DataFormatString="{0:dd/MM/yyyy}" 
                            HeaderText="ET Txn&lt;br&gt;Date" HtmlEncode="False" SortExpression="ETTXNDATE">
                ...
                <asp:SqlDataSource ID="sdsGrid" runat="server" 
                    ConnectionString="<%$ ConnectionStrings:connection %>" 
                    SelectCommand="SELECT TOP (500) ET.LOCID, ET.ACCID, ET.ETID, ACCOUNT.APPID, ACCOUNT.ACCOUNTNAME, '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCH, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, ET.ETTXNDATE, ET.ETSTATUS, ET.ETPENALTY, ET.ETADMIN, ET.OSPRINCIPAL, ET.ETINTEREST, '(' + CAST(ET.ETSTATUS AS VARCHAR(10)) + ') ' + dbo.f_getParamGlobalName('ETSTATUS', ET.ETSTATUS) AS ETSTATUSNAME, ET.ETDATE, ET.ENTRYSOURCE FROM ACCOUNT WITH (NOLOCK) INNER JOIN CUSTOMER WITH (NOLOCK) ON ACCOUNT.LOCIDCUST = CUSTOMER.LOCIDCUST AND ACCOUNT.CUSTID = CUSTOMER.CUSTID INNER JOIN BRANCH WITH (NOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID INNER JOIN ET ON ACCOUNT.LOCID = ET.LOCID AND ACCOUNT.ACCID = ET.ACCID WHERE (ACCOUNT.Deletests = 0) AND (ACCOUNT.ACCSTATUS &lt;&gt; 0)">
                </asp:SqlDataSource>
            </ContentTemplate>
    </asp:UpdatePanel>
...
function ShowET(LOCID, ACCID, ETID){
    window.location = "ETDisplay.aspx?Action=" + GetValueTxtByClientID("<%=txtAction.ClientID%>") + "&LOCID=" + LOCID + "&ACCID=" + ACCID + "&ETID=" + ETID + "&SPV=" + GetValueTxtByClientID("<%=hidSPV.ClientID%>")
}
