...
Partial Class Param_ValidInterest
    ...
    Private Sub SetValidation()
        ...
        oTextBox = CType(fvData.FindControl("INTERESTNAMETextBox"), TextBox)
        If oTextBox IsNot Nothing Then
            SetObjectValidation(oTextBox, False, "uppercase")
            oTextBox.Focus()
        End If
        oTextBox = CType(fvData.FindControl("DESCRIPTIONTextBox"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "uppercase")
        oTextBox = CType(fvData.FindControl("MININTERESTNEWTextBox"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "", "[0-9.]")
        oTextBox = CType(fvData.FindControl("MAXINTERESTNEWTextBox"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "", "[0-9.]")
        oTextBox = CType(fvData.FindControl("MININTERESTUSEDTextBox"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "", "[0-9.]")
        oTextBox = CType(fvData.FindControl("MAXINTERESTUSEDTextBox"), TextBox)
        If oTextBox IsNot Nothing Then SetObjectValidation(oTextBox, False, "", "[0-9.]")
    ...
    Protected Sub sdsForm_Inserted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsForm.Inserted
        ...
        '    lblError.Text = "City Code : " & e.Command.Parameters("@CityCode").Value & " already exists..!"
        'Else
        '    lblError.Visible = False
        '    fvData.Caption = ""
        'End If
