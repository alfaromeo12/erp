...
Partial Class Param_FnFiduciary
    ...
    Private oFiduciaFee As TextBox
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            sdsFiduciary.SelectCommand = sdsFiduciary.SelectCommand & " AND FIDUCIAID=-1"
            gvData.DataBind()
        Else
            hidAction.Value = ""
        End If
    ...
    Private Sub RefreshDataGrid(Optional ByVal row As Integer = 0)
        ...
        If hidAction.Value = "NEW" Then
            ...
            SetObjectValidation(DirectCast(gvData.FooterRow.FindControl("txtNewFiduciaFee"), TextBox), False, "float", "[0-9.,]")
    ...
    Private Sub SetInitialRow()
        ...
        Dim oImageEdit As ImageButton
        dt.Columns.Add(New DataColumn("FIDUCIAID", GetType(String)))
        ...
        dt.Columns.Add(New DataColumn("FIDUCIAFEE", GetType(String)))
        dt.Columns.Add(New DataColumn("REMARK", GetType(String)))
        dr = dt.NewRow()
        dr("FIDUCIAID") = String.Empty
        ...
        oImageEdit = DirectCast(gvData.Rows(Convert.ToInt32(txtTmpSelectedIndex.Text)).FindControl("btnEdit"), ImageButton)
        If oImageEdit IsNot Nothing Then
            oImageEdit.Visible = False
        End If
    ...
    Private Sub BindData()
        Try
            If rdbDealer.Checked = True Then
                If Len(Trim(hidDealer.Value)) > 0 Then
                    sdsFiduciary.SelectCommand = sdsFiduciary.SelectCommand & " AND DEALERID=" & hidDealer.Value
                End If
            End If
            If rdbNonDealer.Checked = True Then
                If Len(Trim(ddlProduct.SelectedValue)) > 0 Then
                    sdsFiduciary.SelectCommand = sdsFiduciary.SelectCommand & " AND PRODUCTID=" & ddlProduct.SelectedValue & " AND AREAID=" & ddlArea.SelectedValue
                End If
            End If
            gvData.DataBind()
        ...
        Catch ex As Exception
            ...
            lblError.Text = "ERROR SEARCH FIN FIDUCIARY: " & ex.Message & "<BR>PLEASE CHECK DETAIL ERROR MESSAGE...!"
        End Try
    End Sub
    ...
    Protected Sub gvData_RowCreated(sender As Object, e As GridViewRowEventArgs)
        If (e.Row.RowType = DataControlRowType.Header) Then
            ...
            AddMergedCells(objgridviewrow, objtablecell, 1, "FIDUCIA FEE (Rp)")
...
#End Region
    ...
    Protected Sub btnInsert_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs)
        ...
        Try
            ...
            oFiduciaFee = DirectCast(gvData.FooterRow.FindControl("txtNewFiduciaFee"), TextBox)
            oDescription = DirectCast(gvData.FooterRow.FindControl("txtNewDescription"), TextBox)

            oSQLParam = objDB.ExecSPReturnParam("FNFIDUCIARYadd", _
                     ...
                     objDB.MP("@FIDUCIAFEE", Data.SqlDbType.Money, oFiduciaFee.Text.ToString, 20), _
            ...
            gvData.DataSourceID = "sdsFiduciary"
            BindData()
            hidAction.Value = "SEARCH"
        Catch ex As Exception
            hidAction.Value = "SEARCH"
            lblError.Visible = True
            lblError.Text = "ERROR INSERT FIN FIDUCIARY: " & ex.Message & "<BR>PLEASE CHECK DETAIL ERROR MESSAGE...!"
        Finally
            objDB.Close()
            objDB = Nothing
        End Try
    ...
    Protected Sub gvData_DataBound(sender As Object, e As System.EventArgs) Handles gvData.DataBound
        ...
        Next
        For Each row As GridViewRow In gvData.Rows
            oTextBox = CType(row.FindControl("txtFiduciaFee"), TextBox)
            If oTextBox IsNot Nothing Then
                SetObjectValidation(oTextBox, False, "float", "[0-9.,]")
            End If
        Next
    ...
    Protected Sub gvData_RowEditing(sender As Object, e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles gvData.RowEditing
        ...
        BindData()
        hidFnFiduciaryID.Value = DirectCast(gvData.Rows(currentRowIndex).FindControl("lblFiduciaIdKey"), Label).Text
        SetObjectValidation(DirectCast(gvData.Rows(currentRowIndex).FindControl("txtPrincipalTo"), TextBox), False, "float", "[0-9.,]")
        SetObjectValidation(DirectCast(gvData.Rows(currentRowIndex).FindControl("txtFiduciaFee"), TextBox), False, "float", "[0-9.,]")
    ...
    Protected Sub sdsFiduciary_Updated(sender As Object, e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsFiduciary.Updated
        If e.Command.Parameters("@retval").Value = -1 Then
            lblError.Visible = True
            lblError.Text = "FIDUCIAID  : " & e.Command.Parameters("@FIDUCIAID").Value & " already exists..!"
        Else
            lblError.Visible = False
            BindData()
        End If
    ...
    Protected Sub gvData_RowUpdating(sender As Object, e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gvData.RowUpdating
        gvData.DataSourceID = "sdsFiduciary"
        BindData()
    End Sub
