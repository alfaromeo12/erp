...
Partial Class Param_DealerMain
    ...
    Private Sub DataSearch()
        If Not IsPostBack() Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND DEALERMAIN.DEALERMAINID=-999"
        End If
        ...
        End If
        If Len(Trim(ddlDealerTypeSearch.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND DEALERTYPE=" & ddlDealerTypeSearch.SelectedValue
        End If
        gvData.DataBind()
    End Sub
    ...
    Protected Sub sdsForm_Inserted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsForm.Inserted
        If e.Command.Parameters("@retval").Value = -1 Then
            ...
            lblError.Text = "Dealer Main Code : " & e.Command.Parameters("@DEALERMAINCode").Value & " already exists..!"
        Else
            lblError.Visible = False
            fvData.Caption = ""
        End If
