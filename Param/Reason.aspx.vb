...
Partial Class Param_Reason
    ...
    Private Sub DataSearch()
        ...
        End If
        If Len(Trim(ddlHardCodeSearch.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND HARDCODE=" & ddlHardCodeSearch.SelectedValue
        End If
        gvData.DataBind()
    End Sub
    ...
    Protected Sub sdsForm_Inserted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsForm.Inserted
        If e.Command.Parameters("@retval").Value = -1 Then
            ...
            lblError.Text = "Reason Code : " & e.Command.Parameters("@REASONCODE").Value & " already exists..!"
        Else
            lblError.Visible = False
            fvData.Caption = ""
        End If
    ...
    Protected Sub ddlHardCodeSearch_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlHardCodeSearch.SelectedIndexChanged
        DataSearch()
    End Sub
