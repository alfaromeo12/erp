<%@ Page Language="VB" MasterPageFile="~/MXPage.master" AutoEventWireup="false" CodeFile="AccRSBatch.aspx.vb" Inherits="Txn_RSBatch" title="Reschedule Per JF (Batch)" EnableEventValidation="FALSE"  MaintainScrollPositionOnPostback="true"%>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphMain" Runat="Server">
    ...
    <asp:SqlDataSource ID="sdsGrid" runat="server" ConnectionString="<%$ ConnectionStrings:connection %>"
        ...
        SelectCommand="SELECT TOP (500) ACCOUNT.LOCID, ACCOUNT.ACCID, ACCOUNT.ACCOUNTNAME, '(' + BRANCH.BRANCHCODE + ') ' + BRANCH.BRANCHNAME AS BRANCH, ACCOUNT.BOOKINGDATE, ACCOUNT.REFNO, dbo.f_FormatAccNo(BRANCH.BRANCHCODE, ACCOUNT.ACCOUNTNO) AS ACCNO, ACCOUNT.PRINCIPALTOTAL, ACCOUNT.EFFECTIVERATE, ACCOUNT.INSTALLMENTREAL, ACCOUNT.APPID FROM ACCOUNT WITH (NOLOCK) INNER JOIN BRANCH WITH (NOLOCK) ON ACCOUNT.BRANCHID = BRANCH.BRANCHID WHERE (ACCOUNT.ACCSTATUS = 1) AND (ACCOUNT.BOOKINGDATE BETWEEN CONVERT (DATETIME, @BOOKINGDATE1, 103) AND CONVERT (DATETIME, @BOOKINGDATE2, 103)) AND (NOT EXISTS (SELECT 'X' AS Expr1 FROM RESCHEDULE WHERE (ACCOUNT.LOCID = LOCID) AND (ACCOUNT.ACCID = ACCID) AND (RSSTATUS = 0))) AND (ACCOUNT.BRANCHID = @BRANCHID) AND (ACCOUNT.TENOR &gt; (SELECT COUNT(*) AS Expr1 FROM INSTALLMENT WITH (NOLOCK) WHERE (ACCOUNT.LOCID = LOCID) AND (ACCOUNT.ACCID = ACCID) AND (DUEDATE &lt; dbo.f_getAppDate())))">
        <SelectParameters>
...
function PrintData() {
    ...
    if (Page_ClientValidate("")) {
        var winref = window.open("../Reports/ReportView.aspx?Back=0&RPT=/DLEAS/Reschedule_InputPerBranch&BOOKINGDATE1=" + BOOKINGDATE1 + "&BOOKINGDATE2=" + BOOKINGDATE2 + "&BRANCHID=" + BRANCHID, "Reschedule_InputPerJF", "height=450,width=700,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes")
        winref.focus();
    }
    return false;
}
