...
Partial Class WO_Inq
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack() Then
            ...
            txtWOFrom.Text = txtAppDate.Value
            txtWOTo.Text = txtAppDate.Value
            txtBookFrom.Text = txtAppDate.Value
            txtBookTo.Text = txtAppDate.Value

            'SetObjectValidation(txtSearch, True)
            SetObjectValidation(txtWOFrom, True, "date", "[0-9/]")
            SetObjectValidation(txtWOTo, True, "date", "[0-9/]")
    ...
    Private Sub DataSearch()
        ...
        If Request.QueryString("ACTION") = "APP" Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND WO.WOSTATUS=0"
        ...
        If rbWOFrom.Checked Then
            If Len(Trim(txtWOFrom.Text)) > 0 And Len(Trim(txtWOTo.Text)) > 0 Then
                Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND (WO.WODATE BETWEEN CONVERT(DATETIME, '" + txtWOFrom.Text + "', 103) AND CONVERT(DATETIME, '" + txtWOTo.Text + "', 103))"
            Else
                Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND (WO.WODATE  BETWEEN CONVERT(DATETIME, '" + txtAppDate.Value + "', 103) AND CONVERT(DATETIME, '" + txtAppDate.Value + "', 103))"
            End If
        End If
        ...
        If Len(Trim(ddlWOStatusSearch.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND WO.WOSTATUS=" & ddlWOStatusSearch.SelectedValue
        End If

        Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " ORDER BY WO.WODATE DESC"
        gvData.DataBind()
    End Sub

    Protected Sub gvData_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvData.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            If DataBinder.Eval(e.Row.DataItem, "WOSTATUS") = 9 Then
