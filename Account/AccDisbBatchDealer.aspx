<%@ Page Language="VB" MasterPageFile="~/MXPage.master" AutoEventWireup="false" CodeFile="AccDisbBatchDealer.aspx.vb" Inherits="Txn_DisbBatchDealer" title="Disbursement per Branch" EnableEventValidation="FALSE" MaintainScrollPositionOnPostback="true" %>
...
<style type="text/css">
            ...
            <tr>
                ...
                <td class="RowStyle">
                    <asp:DropDownList ID="ddlDealer" runat="server" DataSourceID="sdsDealer" 
                        DataTextField="DEALERNAME" DataValueField="DEALERID" CssClass="required">
                    </asp:DropDownList>
                    <asp:SqlDataSource ID="sdsDealer" runat="server" 
                        ConnectionString="<%$ ConnectionStrings:connection %>" 
                        
                        
                        SelectCommand="SELECT NULL AS DEALERID, NULL AS DEALERCODE, '[SELECT DEALER]' AS DEALERNAME UNION ALL SELECT DEALERID, DEALERCODE, DEALERNAME + ' (' + DEALERCODE + ')' AS DEALERNAME FROM DEALER WHERE EXISTS (SELECT 'X' AS Expr1 FROM ACCOUNT WITH (NOLOCK) INNER JOIN COLLATERAL WITH (NOLOCK) ON ACCOUNT.LOCID = COLLATERAL.LOCID AND ACCOUNT.ACCID = COLLATERAL.ACCID WHERE (COLLATERAL.DEALERID = DEALER.DEALERID) AND (ACCOUNT.ACCSTATUS &lt;&gt; 0) AND (ACCOUNT.DISBURSESTS = 0)) ORDER BY DEALERNAME">
                    </asp:SqlDataSource>
                    <asp:RequiredFieldValidator ID="reqDealer" runat="server" 
                        ControlToValidate="ddlDealer" Display="None" 
                        ErrorMessage="[DEALER] must be entry...!" SetFocusOnError="True"></asp:RequiredFieldValidator>
                    <ajaxToolkit:ValidatorCalloutExtender ID="reqDealer_ValidatorCalloutExtender" 
                        runat="server" HighlightCssClass="ErrHighLight" TargetControlID="reqDealer">
                    </ajaxToolkit:ValidatorCalloutExtender>
                </td>
            </tr>
            <tr>
    ...
    <ajaxToolkit:TabContainer ID="TabContainerDisb" runat="server" ActiveTabIndex="1" 
        ...
        DataSourceID="sdsSummary" Width="100%" AllowSorting="True">
        <Columns>
    ...
    <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="Fiducia&lt;br&gt;(d)" 
                SortExpression="FIDUCIAFEE">
                <ItemTemplate>
                    <asp:Label ID="Label6" runat="server" Text='<%# Eval("FIDUCIAFEE", "{0:N}") %>'></asp:Label>
                </ItemTemplate>
                <ItemStyle HorizontalAlign="Right" />
            </asp:TemplateField>
    ...
    <asp:SqlDataSource ID="sdsGrid" runat="server" ConnectionString="<%$ ConnectionStrings:connection %>"
        ...
        <SelectParameters>
            ...
            <asp:ControlParameter ControlID="ddlDealer" Name="DEALERID" 
...
function PrintData() {
    ...
    var DEALERID = GetValueTxtByClientID("<%=ddlDealer.ClientID%>");
    ...
    if (Page_ClientValidate("")) {
        var winref = window.open("../Reports/ReportView.aspx?Back=0&RPT=/DLEAS/Disbursement_InpurPerDealer&BOOKINGFROM=" + BOOKFROM + "&BOOKINGTO=" + BOOKTO + "&COLLSTS=" + sReceive + "&DEALERID=" + DEALERID,"DisbursementPerDealer", "height=450,width=700,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes");
        winref.focus();
    }
    return false;
}
