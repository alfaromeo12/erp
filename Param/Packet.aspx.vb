...
Partial Class Param_Packet
    ...
    Private Sub SetValidation()
        ...
        oTextBox = CType(fvData.FindControl("txtPacketCode"), TextBox)
        If oTextBox IsNot Nothing Then
            SetObjectValidation(oTextBox, False, "uppercase")
            oTextBox.Focus()
        End If
        oTextBox = CType(fvData.FindControl("txtPacketName"), TextBox)
    ...
    Protected Sub sdsForm_Inserted(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.SqlDataSourceStatusEventArgs) Handles sdsForm.Inserted
        If e.Command.Parameters("@retval").Value = -1 Then
            ...
            lblError.Text = "Packet Code : " & e.Command.Parameters("@PacketCode").Value & " already exists..!"
        Else
            lblError.Visible = False
            fvData.Caption = ""
        End If
