Partial Class Txn_ParipasuEdit
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Request.QueryString("ACTION") = "PARIPASU" Then
                If Len(Trim(hidACCIDPARIPASU.Value)) > 0 Then
                    ParipasuAdd()
    ...
    Private Sub ParipasuAdd()
        Dim objDB As New DBX
        Try
            lblError.Visible = False
            objDB.ExecSP("PARIPASUAdd", _
                objDB.MP("@LOCID", Data.SqlDbType.Int, hidLOCID.Value), _
                objDB.MP("@ACCID", Data.SqlDbType.BigInt, hidACCID.Value), _
                objDB.MP("@LOCIDPARIPASU", Data.SqlDbType.Int, hidLOCIDPARIPASU.Value), _
                objDB.MP("@ACCIDPARIPASU", Data.SqlDbType.BigInt, hidACCIDPARIPASU.Value), _
                objDB.MP("@USERID", Data.SqlDbType.Int, Request.Cookies("UID").Value))
            gvParipasu.DataBind()
        Catch ex As Exception
            lblError.Visible = True
            lblError.Text = "ERROR ADDING PARIPASU...!<BR>PLEASE CHECK DETAIL ERROR MESSAGE:<BR>" & ex.Message
            lblError.Focus()
        Finally
            objDB.Close()
            objDB = Nothing
            ...
            For Each row As GridViewRow In gvParipasu.Rows
                ...
                If oCheckBox IsNot Nothing Then
                    If oCheckBox.Checked Then
                        strLOCID = gvParipasu.DataKeys(row.RowIndex).Values("LOCIDX").ToString()
                        strACCID = gvParipasu.DataKeys(row.RowIndex).Values("ACCIDX").ToString()
                        strPARIPASUID = gvParipasu.DataKeys(row.RowIndex).Values("PARIPASUID").ToString()
                        'Response.Write("<BR>")
                        'Response.Write("<BR>")
                        'Response.Write("<BR>")
                        'Response.Write(strACCID & "<BR>")
                        'Response.Write(strPARIPASUID & "<BR>")
