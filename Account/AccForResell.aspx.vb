...
Partial Class Txn_AccForResell
    ...
    Private Sub FormValidation()
        ...
        SetObjectValidation(txtDPMin, False, "integer", "[0-9]")
    ...
    Private Sub DataSearch()
        ...
        If cbPastDue.Checked Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND NOT EXISTS (SELECT 'X' FROM INSTALLMENT WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=INSTALLMENT.LOCID AND v_AccountForJFResell.ACCID=INSTALLMENT.ACCID AND PAIDSTS=0 AND DUEDATE<=dbo.f_getAppDate())"
        End If
        ...
        If cbNewAcc.Checked Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND NOT EXISTS (SELECT 'X' FROM v_AccountForJFNew WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=v_AccountForJFNew.LOCID AND v_AccountForJFResell.ACCID=v_AccountForJFNew.ACCID)"
        End If
        ...
        If Len(Trim(ddlProductID.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND EXISTS (SELECT 'X' FROM COLLATERAL WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=COLLATERAL.LOCID AND v_AccountForJFResell.ACCID=COLLATERAL.ACCID AND COLLATERAL.PRODUCTID=" & ddlProductID.SelectedValue & ")"
        End If

        If Len(Trim(ddlCondition.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND EXISTS (SELECT 'X' FROM COLLATERAL WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=COLLATERAL.LOCID AND v_AccountForJFResell.ACCID=COLLATERAL.ACCID AND COLLATERAL.CONDITION=" & ddlCondition.SelectedValue & ")"
        End If

        If Len(Trim(ddlPurpose.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND EXISTS (SELECT 'X' FROM COLLATERAL WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=COLLATERAL.LOCID AND v_AccountForJFResell.ACCID=COLLATERAL.ACCID AND COLLATERAL.PURPOSEID=" & ddlPurpose.SelectedValue & ")"
        End If

        If Len(Trim(ddlBPKBStatus.SelectedValue)) > 0 Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND EXISTS (SELECT 'X' FROM COLLATERAL WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=COLLATERAL.LOCID AND v_AccountForJFResell.ACCID=COLLATERAL.ACCID AND COLLATERAL.BPKBSTATUS=" & ddlBPKBStatus.SelectedValue & ")"
        End If

        If rbEarlyExclude.Checked Then
            Me.sdsGrid.SelectCommand = Me.sdsGrid.SelectCommand & " AND NOT EXISTS (SELECT 'X' FROM INSTALLMENT WITH(NOLOCK) WHERE v_AccountForJFResell.LOCID=INSTALLMENT.LOCID AND v_AccountForJFResell.ACCID=INSTALLMENT.ACCID AND INSTALLMENT.DUEDATE>dbo.f_getAppDAte() AND (INSTALLMENT.PRINCIPALPAID>0 OR INSTALLMENT.INTERESTPAID>0))"
        End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
            ...
            For Each row As GridViewRow In gvData.Rows
                ...
                If oCheckBox IsNot Nothing Then
                    If oCheckBox.Checked Then
                        ...
                        'strACCNO = gvData.DataKeys(row.RowIndex).Values("ACCNO").ToString()
                        ...
                        intRetVal = objCore.JFREQUESTACCAdd(ConvertDate(txtRequestDate.Text), ddlJF.SelectedValue, _
                             strLOCID, strACCID, txtInstallmentDay.Text, 2, "", 0, 1, Request.Cookies("UID").Value)
                        If intRetVal = -1 Then
                        ...
                        ElseIf intRetVal = -4 Then
                            lblMessage.Visible = True
                            lblMessage.Text = "ACCOUNT " & oTextBox.Value & " ALREADY THIS REQUEST DATE..!"
                            Exit For
        ...
        Catch ex As Exception
            lblMessage.Visible = True
            lblMessage.Text = "ERROR SAVE DATA REQUEST JF RESELL: " & ex.Message & "<br>PLEASE CHECK DETAIL ERROR MESSAGE...!"
            lblMessage.Focus()
        Finally
            objCore = Nothing
