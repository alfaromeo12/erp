<%@ Page Title="General Transaction Approval" Language="VB" MasterPageFile="~/MXPage.master" AutoEventWireup="false" CodeFile="TxnInputApproval.aspx.vb" Inherits="Txn_TxnInputApproval" EnableEventValidation="false" %>
        ...
        border-right-style: outset; border-left-style: outset; border-bottom-style: outset" 
            ...
            <asp:GridView ID="gvData" runat="server" AllowPaging="True" AllowSorting="True" 
                ...
                <Columns>
                    ...
                    <asp:BoundField DataField="BRANCHNAME" HeaderText="Branch&lt;BR&gt;Txn" ReadOnly="True" 
            ...
            <asp:SqlDataSource ID="sdsData" runat="server" 
                ...
                SelectCommand="SELECT TOP (500) TXN.LOCIDTXN, TXN.TXNID, TXN.TXNDATE, TXN.VALUEDATE, '(' + BRANCH.BRANCHCODE + ')' + BRANCH.BRANCHNAME AS BRANCHNAME, ACCOUNT.ACCOUNTNAME, dbo.f_FormatAccNoByID(ACCOUNT.BRANCHID, ACCOUNT.ACCOUNTNO) AS ACCNOFORMAT, '(' + BOOKCODE.BCCODE + ')' + BOOKCODE.BCNAME AS BCNAME, TXN.REFNO, TXN.TXNSTATUS, (SELECT ISNULL(SUM(AMOUNT), 0) AS Expr1 FROM TXNJOURNAL WITH (NOLOCK) WHERE (TXN.LOCIDTXN = LOCIDTXN) AND (TXN.TXNID = TXNID) AND (DRCR = 'D')) AS AMOUNT FROM TXN INNER JOIN BRANCH WITH (NOLOCK) ON TXN.BRANCHID = BRANCH.BRANCHID INNER JOIN BOOKCODE WITH (NOLOCK) ON TXN.BCID = BOOKCODE.BCID LEFT OUTER JOIN ACCOUNT WITH (NOLOCK) ON TXN.LOCID = ACCOUNT.LOCID AND TXN.ACCID = ACCOUNT.ACCID WHERE (TXN.TXNSTATUS = 0)">
            </asp:SqlDataSource>
        </asp:View>
        <asp:View ID="vEntry" runat="server">
           <asp:DataList ID="dlGeneralTrans" runat="server" DataSourceID="sdsForm" 
                Width="100%" Caption="TRANSACTION INFO" >
                <ItemTemplate>
                    <table cellpadding="0" cellspacing="0" width="100%">
                        <tr>
                        ...
                        <tr class="RowStyle">
                            ...
                            <td>
                                (<asp:Label ID="Label1" runat="server" 
                                    style="font-weight: 700; text-decoration: underline" 
                                    Text='<%# Eval("FINCATCODE") %>' CssClass="ErrHighLight"></asp:Label>)
                                <asp:Label ID="Label2" runat="server" 
                                    style="font-weight: 700; text-decoration: underline" 
                                    Text='<%# Eval("FINCATNAME") %>' CssClass="ErrHighLight"></asp:Label>
                            </td>
                        </tr>
                ...
                <asp:Panel ID="pnlControl" runat="server" CssClass="HeaderStyle" Width="99%">
                 ...
                 <Animations>
                    ...
                    <asp:Button ID="btnApprove" runat="server" CommandName="Approve" 
                        Style="background-position: left; background-image: url(../images/approve.gif); cursor: pointer; background-repeat: no-repeat; text-align: right; " 
                        Text="Approve" ToolTip="Approve Transaction" Width="72px" />
                    &nbsp;<asp:Button ID="btnReject" runat="server" CommandName="Reject" 
                        Style="background-position: left; background-image: url(../images/reject.png); cursor: pointer; background-repeat: no-repeat; text-align: right; " 
                        Text="Reject" ToolTip="Reject Transaction" Width="72px" />
